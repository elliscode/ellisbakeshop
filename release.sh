...
if $lambda; then
    ...
    cd ../
    aws lambda update-function-code --function-name=ellisbakeshop-api --zip-file=fileb://lambda-release-${TIMESTAMP}.zip --no-cli-pager
fi

if $s3; then
    aws s3 sync s3 s3://daniel-townsend-ellisbakeshop --exclude "*env.js" --exclude "*.DS_Store" --delete
fi