...
def parse_cookie(input):
    ...
    for cookie in cookies:
        ...
        if cookie_name == "ellisbakeshop-auth-token":
...
def ios_cookie_refresh_route(event, user_data, body):
    ...
    return format_response(
        ...
        headers={
            "Set-Cookie": f'ellisbakeshop-auth-token={token_data["key2"]}; Domain=.ellisbakeshop.com; Expires={date_string}; Secure; HttpOnly',
        },
    )
...
def otp_route(event):
    ...
    if otp_data is None or otp_data["expiration"] < int(time.time()):
        ...
        message = {
            "phone": f"+1{phone}",
            "message": f"{otp_data['otp']} is your ellisbakeshop.com one-time passcode\n\n@ellisbakeshop.com #{otp_data['otp']}",
        }
        print(message)
        sqs.send_message(
